...
- **(B) Change the `dag_id` (the Airflow identity)** — a **guided, deploy‑state‑aware migration, not a rename.** Airflow 3 has **no `dag_id` rename** (`PATCH /dags/{id}` only toggles `is_paused`): a new `dag_id` is a **brand‑new DAG with no run history**, and because the deployed file is `{dag_id}.py` the old file is **orphaned**. Today `dag_id` is freely editable in the DAG form with **zero guard** — this feature **intercepts** that edit and routes it by the state of the *current* `dag_id`:
  ...
  - **Deployed, idle (no active run):** a **"Rename & redeploy"** dialog that states the consequences up front — a new DAG `{new_id}` is created **paused** with **fresh history**, and the old `{old_id}` history **does not carry over**. Order: validate + collision → regenerate → **deploy `{new_id}.py` and verify it registers** (tri‑state §6.5.4) → **then** reconcile the old DAG (**write‑new‑then‑remove‑old**, so there is never a zero‑file gap; the new DAG is paused so the brief two‑DAG window is harmless). Old‑DAG handling is the user's choice, defaulting to the **non‑destructive** option:
    - **Keep history (default):** pause `{old_id}` and **remove `{old_id}.py`** so it isn't re‑parsed; Airflow retains the old run history (the dag becomes fileless/`stale`, still viewable via `exclude_stale=false`). **The list must not wait for Airflow to agree ✅:** keeping the history means keeping the `DagModel` row, so `GET /dags?exclude_stale=true` goes on returning `{old_id}` until the dag‑processor's next scan marks the fileless DAG stale — one full `dag_dir_list_interval` (~300 s), and *never* when the deploy target isn't the folder that processor scans. For that window the rename looks like it did nothing (both ids listed), and the state is invisible to the safety nets (`find_orphans` joins on the file the retire just deleted), leaving Delete — which purges the very history the user chose to keep — as the only remedy. So the retire records the id in the journal (`retired/` marker) and `GET /dags` filters it out (`deploy.drop_retired`) until Airflow catches up. Two independent releases keep that honest: the marker is dropped the moment a `{old_id}.py` exists again (re‑deploy, rename back, or a hand‑written DAG claiming the freed id), and it expires by TTL regardless.
...
- **Chained renames — a retire *queue*, not a slot ✅.** Renaming again while the previous deploy is still awaiting registration (Airflow takes ~300 s, so this is an ordinary user rhythm) supersedes the first journal entry and **inherits** its unfinished retire intent. The entry carries a queue (`retire` + `retire_also`), so a second intent is appended rather than dropped: dropping it left the *first* dag_id deployed, registered and — the retire being what would have paused it — **unpaused**, running on its schedule beside the newest one, while the deploy banner reported a clean success. The reconciler drains the queue, recording which ids it has already retired so a mid‑queue retry never retires (or re‑audits) the same DAG twice; the "never retire the DAG this deploy just wrote" guard applies per queued id.
- **A timed‑out rename's retire is inherited by the next deploy ✅.** When the budget elapses the retire is *skipped*, not performed, and the old dag_id is left active. `supersede` only scans **open** entries, so that intent used to be stranded in a terminal entry where nothing could reach it — and recovery closed behind it, since *Keep waiting* (`reopen_expired`) refuses once a newer deploy of the flow is in flight and the editor only ever re‑attaches to `latest_for_afdag`. Journaling a new deploy therefore also scans recently‑terminal entries of the same flow whose retire was skipped for a **budget** reason (never for a decision — a failed import, a foreign owner, a user pause) and adopts their intent, marking it consumed so it is inherited exactly once.
...
- **Viewer role ✅ (2026‑08‑15).** `JUPYTERLAB_AIRFLOW_ROLE=editor|viewer` (`config.studio_role()`). A **viewer** reads everything — DAG list, runs, task logs, import errors, the CODE tab, live validation — but cannot run any privileged action: deploy/undeploy/retire/rollback, trigger/pause/stop‑run/clear/delete, or write Variables/Connections. **The privileged set is *defined as* the audited set:** the gate lives in `_AirflowHandler.respond` and keys off the same `audit_action` marker that drives the audit trail, so the two cannot drift — a future mutating handler cannot be audited‑but‑ungated or gated‑but‑unaudited. It runs **before** `fn`, so a denied deploy writes nothing. A refusal is itself audited with `outcome="denied"` — an attempted privileged action by a view‑only user is exactly what the trail is for. Unset defaults to **editor**, so every existing install is unchanged (opt‑in); an **unrecognised value falls back to viewer**, because a typo in a permission setting must never grant the privilege it was meant to withhold. On JupyterHub, set it per user at spawn via `c.Spawner.environment` — the same injection point this section already recommends for per‑user Airflow creds. `GET /capabilities` reports `{role, can_edit}` so the UI can present a coherent read‑only mode; that endpoint is **advisory only** — a client that ignores it gains nothing, since the decision is enforced server‑side. **Known boundary:** a `.afdag` is an ordinary file in the user's own Jupyter workspace, so a viewer can still edit it in the text editor or via the Contents API — gating the Studio canvas is a UX guardrail, not a security boundary. What *is* enforced is that nothing they change can reach Airflow.
...
- **Collision protection** (§6.5.3) **✅ (2026‑08‑15)**: refusing to overwrite a non‑Studio file already existed, but it left a real hole — a *Studio‑managed* file was assumed safe to overwrite, so two `.afdag` documents choosing the same `dag_id` map to the same `{dag_id}.py` and **the second silently clobbered the first**, backup and all. `deploy_dag` now runs a pre‑write ownership gate that refuses when **(a)** `{dag_id}.py` is Studio‑managed but its header `afdag_id` belongs to a *different* flow, or **(b)** Airflow already serves that `dag_id` from a different `fileloc` (typically a hand‑written DAG). It returns the same `{deployed: false, errors:[…]}` shape as the validation/provider/variable gates, so the editor renders it with no new UI, and runs **before** the variable sync so a refusal leaves nothing behind. No‑op when Airflow is unreachable, matching the variables gate. A flow re‑deploying its own `dag_id` is unaffected. A *deployed* file with no `afdag_id` (pre‑provenance) is still overwritable — it is unattributable, blocking would strand those flows, and the overwrite is backed up. Still open: the "modified outside Studio" drift flow.
...
- **Audit ✅ (2026‑06‑23).** Every mutating action — **deploy · trigger · pause · unpause · stop‑run · clear · delete · rollback · retire** — emits a structured `{ts, user, action, dag_id, correlation_id, outcome, detail?}` JSON line on the dedicated `jupyterlab_airflow.audit` logger (`audit.py`), stamped with the authenticated Jupyter user (resolved from `self.current_user`; the Hub user under JupyterHub). Wired once into the base handler's `respond(…, audit_action=, audit_dag_id=)` so each mutating handler opts in and read‑only reads stay un‑audited; a dry‑run clear (preview) isn't audited. Records are JSON‑serialized (no log injection) and log the *action only*, never the request body (so a trigger `conf` with secrets isn't recorded); `detail` carries a trimmed error message on failure. **Actually written ✅ (2026‑08‑17).** The trail used to be *inert by default*: the records were emitted correctly, but `jupyterlab_airflow.audit` sits at `NOTSET` under a root logger Jupyter leaves at `WARNING` with no handlers, so every `INFO` record was discarded. Nothing warned about it — the feature looked present, the tests passed (their fixture attaches its own handler, which is precisely what production lacked), and an operator would have found out only when they went looking for a record that was never written. `audit.configure()` now runs at extension load and attaches a **rotating file handler** (`<data_dir>/airflow-studio/audit.log`, 5 MB × 5), created at **0600** before the handler opens it — it names users and what they did, and a chmod afterwards would leave a world‑readable window. One JSON object per line, no log prefix, so it stays parseable line‑by‑line. Two rules keep it from fighting a deployment that already routes the logger: if handlers are **already attached** we add none of our own, and the level is only raised when it is `NOTSET` — an explicit level is a decision, `NOTSET` is just the default that made handlers useless. `JUPYTERLAB_AIRFLOW_AUDIT_LOG` sets the path or `off` disables the file; an unwritable path warns loudly and lets the server start, because "no records" now means "not recorded" rather than "nothing happened". *(This landed before full per‑user creds injection, exactly as planned — attribution is correct per server process; see the multi‑user note above.)*
...
### 15.0 Visual language — Data4Now ✅

Implemented from the `Airflow Studio.dc.html` design and the `data4now-design` skill. **All of it lives in `style/d4n-tokens.css`**; `afdag.css` and `base.css` contain **no literal colour values**, so re‑theming is a one‑file change.

**Palette.** Navy `--d4n-navy` #0F3D6E (primary: headings, solid buttons, active tab) · navy‑deep #082B52 (hover/pressed) · teal `--d4n-teal` #1FA0A0 (structural accent: eyebrows, focus ring, selection, the palette item's leading bar) · magenta `--d4n-magenta` #E63558 (**a single point of emphasis only** — destructive actions and validation errors; never a third structural colour). Neutrals are cool, navy‑tinted greys (`ink → graphite → slate → steel → mist → fog → paper`), not plain greys — plain greys read as unbranded beside the navy. Status: success #1F8A5E, warning #C97C0A, danger = magenta, info = teal, each with a tint background + border.

**Type.** Montserrat 600/700 (display: eyebrows, titles, labels, buttons) · Roboto (body, form controls) · JetBrains Mono (`dag_id`, `task_id`, paths, timestamps, generated code, logs). All three are **bundled as latin‑subset woff2** under `style/fonts/` rather than fetched from Google Fonts, because the extension ships into offline and air‑gapped deployments where a remote font request simply fails; both licenses (OFL‑1.1, Apache‑2.0) permit redistribution.

**Recurring elements.** *Eyebrow* — 10px uppercase Montserrat in teal, `0.14em` tracking, above every section. *Pill* — uppercase micro‑label on a status tint, used for node readiness, validation counts, variable/connection scope, run state. *Card* — 8–10px radius, 1px `--d4n-border-1`, optional sunken header strip. *Dashed affordance* — "add" actions (Add note, Add notifier) are dashed to read as an empty slot rather than an existing action. Radii 6px (controls) / 10px (cards) / pill. Shadows are navy‑tinted, not neutral black. Motion is a single 200ms `cubic-bezier(.2,0,.1,1)`, and is disabled wholesale under `prefers-reduced-motion`.

**Light / dark.** The design is a fixed light theme, but JupyterLab lets the user pick. The tokens split: **brand hues are constant** (a teal focus ring is teal in both), while **neutrals default to the design's exact values and flip to `--jp-*` under `body[data-jp-theme-light='false']`**. So JupyterLab's light theme reproduces the design, and a dark theme keeps the layout and brand accents while adopting that theme's surfaces. Three brand tokens need a dark variant because navy is itself dark and would vanish on a dark surface — `--d4n-fg-heading`, `--d4n-solid-bg`, `--d4n-solid-bg-hover` lift toward the lighter end of the brand blue. Status tints swap pastel washes for low‑alpha tints of the same hue. **Generated code and task logs stay dark in both themes** — the design treats code as an inset terminal.

**Accessibility.** No state is signalled by colour alone: every status pill and palette availability state carries a glyph or text label as well. Focus is a 3px teal ring via `:focus-visible`, so it appears for keyboard users and not on mouse click.
...
```
┌───────────────────────────────────────────────────────────────────────────────┐
│ AIRFLOW STUDIO                                        (teal eyebrow, §15.0)      │
│ my_dag  4 nodes  (✓ NO ERRORS)        [▣TaskFlow│Traditional] ≣Tidy Save ▶Deploy │
│   (left = what the flow IS · right = what you DO to it; picking a syntax opens CODE) │
│   (the dag_id above is click-to-edit — §15.11; renaming the FILE is the file browser's job) │
├──── OPERATORS ───«─┬─────────────── CANVAS ───────────────┬─»── INSPECTOR ───────┤
│ 🔍 Search…         │                                      │ (DAG) NODE INFO VARS │
│                    │                                      │ CONNS NOTIFY CODE SAVED  ← pills, wrap to 2 rows │
│ ▾ PYTHON / BASH    │        ┌────────────────────┐        │ ─────────────────────│
...
```
Built ✅: beyond the drop hint, a dismissible **3‑step coachmark** (`Coachmark`) pinned top‑centre of the canvas walks a first‑time user **add task → configure → deploy**. It advances from state (step 1→2 when the first task lands; finishes when a Deploy starts) with *Next*/*Done* + *Skip tour*, and is shown **once per browser** (a `localStorage` flag), so it never nags a returning user (§7). Restyled per §15.0: the card is a 10px‑radius white panel with a teal eyebrow and a `--d4n-shadow-md`, and step progress reads as three short **bars** rather than dots — bars show "steps remaining" at a glance and give the filled state somewhere to go.
...
**Server‑completed deploys — what the banner means now ✅ (§6.5.4).** When the server journals the deploy (`lifecycle.reconciled === true`), the banner **observes**; it performs nothing. Three affordances change meaning accordingly:

```
 ② ┌ ⏳ Waiting for Airflow to pick it up… This continues even if you close    ┐
   │   this tab.                            [ Keep waiting ]        [ × ]      │
 ②′┌ ⏳ Still processing — the server keeps finishing this deploy either way.  ┐
   │                                        [ Keep waiting ]        [ × ]      │  ← re-arms the SERVER
 ④ ┌ Stop finishing this deploy? ─────────────────────────────────────────┐
...
* **[ × ] Dismiss** on a journaled deploy is a real stop, not "stop looking" — closing the tab no longer stops anything — so it confirms first (④) and then `POST deploy/lifecycle/cancel`. The cancel is **claim‑respecting**: if a sweep is mid‑step the server records the stop durably and honours it before its next transition (`pending_step: true`), rather than blind‑writing over the entry and having that sweep silently undo it.
* **[ Keep waiting ]** never takes the work back into the tab. While the lifecycle is live it re‑attaches the observer; after an `expired` outcome it calls `POST deploy/lifecycle/resume`, which re‑arms the journal entry server‑side (fresh budget, budget‑skipped steps back to `pending`). This matters because on the journaled path the rename's retire intent lives **only** in the journal — finishing the tail in the browser would unpause and trigger the new DAG while the renamed‑away one is still on disk, registered and unpaused. Refused when the deploy is not `expired` or a newer deploy of the flow is in flight; the banner then says so and points at the Manager.
* A **paused** DAG (from anywhere in Studio) vetoes the deploy's pending unpause **and** trigger, and the terminal banner reads *"left paused at your request."* Deliberate intent beats a deploy still in flight.
...
```
 ...
 └────────────────────────────────────────────────────┘
  ...
  after a keep-history rename HELLO → HELLOO, the retired id is gone AT ONCE —
  not ~300 s later when Airflow's dag-processor marks the fileless row stale:

 │ ◐ HELLOO         @daily                 ⏸  ▶     🗑 │   ← only the new id
 │ ⨯ HELLO          (paused, no file)      ⏸  ▶     🗑 │   ✅ filtered out (drop_retired)
```
Built ✅: list (search/tag filter, `exclude_stale`), pause, trigger, run‑status, `has_import_errors` badge + import‑errors panel, delete (file‑then‑history). **Retired ids leave the list immediately ✅ (§6.1.8(B)):** a keep‑history retire keeps the `DagModel` row on purpose (that row *is* the history), so Airflow kept listing the old dag_id beside the new one for a whole `dag_dir_list_interval` — and forever when the deploy target isn't the scanned folder — making a rename look like a no‑op. The list now filters ids the journal records as retired (`drop_retired`), releasing them as soon as a `{dag_id}.py` exists for them again (or by TTL). **Friendly import errors + "Open in Studio to fix" ✅ (§7):** each import error renders a plain‑language card (`explainImportError`) — filename, a friendly title/summary, a `pip install …`/fix hint, the raw trace tucked under *Show technical details* — with a one‑click **Open in Studio to fix** that calls `dags/source` (resolve the deployed `.py` → its source `.afdag` via the `afdag_id` provenance ↔ Contents‑root join) and opens it in the Studio factory; when the source is gone/pre‑provenance it explains why instead. **Trigger ✅:** a no‑params DAG triggers instantly; a DAG with `params` opens the conf dialog (§15.10). **Stop‑run ✅:** a **stop** link on a `running`/`queued` run in the drill‑down (§15.9) `PATCH`es it to `failed` (§6.6). **Orphan banner ✅:** the manager calls `dags/orphans` on every refresh and renders a warn‑coloured banner of deployed DAGs whose source `.afdag` was deleted (§6.5.6 / §15.13), each with *Undeploy & purge* / *Keep*. **Auto‑refresh ✅ (2026‑08‑17):** the panel re‑reads Airflow every 15s, so a scheduled run starting, a task failing, or the reconciler finishing a deploy shows up without anyone pressing refresh. Four details keep it from being a nuisance rather than a feature: a background tick shows **no spinner** (a "Loading…" flash every 15s reads as instability); a failed tick **leaves the last good list on screen** rather than replacing a working view with an error banner; it **stands down while any dialog is open**, because a list re‑ordering under a confirm is how someone confirms the wrong DAG; and it **skips a hidden tab** entirely, refreshing immediately on `visibilitychange` instead, so a forgotten background tab is not a permanent 15‑second heartbeat against Airflow. Expanded run drill‑downs are **re‑read in place, never collapsed** — watching a run progress is the main thing auto‑refresh is for, and clearing them on a timer would make it the one thing you cannot do. The orphan sweep walks the whole Contents tree, so it rides a slower cadence (every 4th tick). **Data4Now restyle ✅ (§15.0):** the panel leads with a teal `APACHE AIRFLOW` eyebrow over a Montserrat 19px navy **Pipelines** title; a `N DAGS` count eyebrow separates the alert cards from the list; DAG rows became hover‑tinted cards with a status dot, a Montserrat name and a slate meta line, and their pause/trigger/delete icon actions pick up per‑action hover tints (neutral / teal / magenta) so the destructive one is distinguishable before you click it. Import‑error and orphan banners are now tinted **cards** with the `pip install …` line boxed in mono. Run states render as tinted pills rather than solid fills, with the live run's dot pulsing. A footer bar names the auto‑refresh interval and the `/api/v2` base.
...
```
 ...
 └──────────────────────────────────────────┘
```
Built ✅: task instances + states, task logs (a single fetch of the task's **current try**, rendered as one raw `<pre>`), clear/retry (dry‑run preview → confirm), mark success/failed/skipped, and (✅) a **stop** link on a `running`/`queued` run that `PATCH`es it to `failed` (→ scheduler terminates its tasks, §6.6 / §8.8) behind a confirm — distinct from Clear/Retry (re‑run) and Mark‑state. **Friendly log viewer ✅ (§6.6):** the raw `<pre>` is replaced by a structured viewer (`LogViewer.tsx`) — **per‑level colour + an error left‑bar** (non‑color‑only), **traceback‑as‑error + autoscroll‑to‑first‑error**, an **attempt selector** (try 1…N, re‑fetched over the existing API), **search** + **errors‑only** filter, **Copy/Download**, a **Wrap** toggle, and a load/error state distinct from content. Levels come from Airflow 3's **structured events** when present (`client.get_task_logs` passes `events?: {event, timestamp?, level?, logger?}[]` through — §13 Q16 ✅, 2026‑06‑24), with **client‑side text `classifyLine` as the per‑event + plain‑text fallback**; the flattened `content` stays for Copy/Download (back‑compatible). `Overlay` gained Escape‑close + focus‑on‑open. **Data4Now restyle ✅ (§15.0):** the modal is a 10px‑radius panel on a navy scrim with `--d4n-shadow-xl`, a teal eyebrow + Montserrat title, and a segmented attempt selector on a sunken track. **The log body is dark in both themes** (`--d4n-code-bg`) — the design treats logs, like generated code, as an inset terminal — with levels drawn from `--d4n-log-*` tuned for that surface. The previous per‑row error *tint* was dropped in the process: the status‑tint tokens are light‑theme pastels, and a pastel wash would punch a white hole in a dark log; the error left‑bar remains, so the signal is still not colour‑only. ndjson live‑tailing stays deferred 🔭. *(Native grid/Gantt/XCom stay in Airflow’s own UI — NG3; optional deep‑link.)*
...
> **Fixed — the deploy lifecycle is now server‑side ✅ (2026‑08‑16).** It used to run entirely inside the open `.afdag` widget: one `if (state === 'registered')` gate, the retire intent in a bare `React.useRef`, and no server background work at all. Closing the tab while Airflow was still scanning abandoned the migration silently — the new DAG stayed **paused** and the old dag_id stayed **deployed and active**. Measured on a real occurrence: file written 18:56:09, page reloaded 18:59:42, Airflow registered 19:01:05 — the widget died *85 s before registration*, which is also why the earlier 420 s‑budget fix could not help: it hardened a timeout path that was never taken.
>
> Now `deploy_dag` journals the pending lifecycle (`journal.py`) and a reconciler (`reconciler.py`) finishes it: **wait for registration → retire the renamed‑away dag_id → unpause → trigger**. The browser is demoted to an *observer* — it polls a read‑only projection and performs nothing. Key properties: the timer exists **only while a deploy is in flight** (an idle server schedules nothing); every Airflow call runs on a dedicated executor, never on the IOLoop; the run id is derived from the deploy's `correlation_id`, so a retried trigger cannot double‑fire; retire is guarded by `expect_afdag_id` so it can never remove a *different* flow's freshly deployed file; `can_edit()` is re‑checked every sweep (fail‑closed); and every action is audited with `via="reconciler"` and the **human** as `user` — the reconciler is a mechanism, not a principal. Past the budget nothing is unpaused or triggered: a late unpause is not completing a deploy, it is fighting the user. A tab from *before* the upgrade posts a bare IR, which is deliberately **not** journaled — gating on the envelope prevents one deploy having two performers. Off switch: `JUPYTERLAB_AIRFLOW_RECONCILER=off`.
>
> **Verified end‑to‑end with the browser tab closed:** rename + deploy posted, tab shut, and the server alone drove registration (~260 s) → retire → unpause → trigger to a successful run.

> **Fixed — the three ways a renamed‑away dag_id survived the rename ✅ (2026‑08‑17).** All three ended in the same reported symptom (the old id still in the list, in two of them still *unpaused and scheduled*), and none was visible in the UI:
>
> 1. **Keep‑history retire, id still listed.** The retire deliberately keeps the `DagModel` row, and `GET /dags` returned it until the dag‑processor next scanned (~300 s; never, when the deploy target isn't that folder). Fixed by making the list authoritative — a `retired/` journal marker + `drop_retired`, released when a file for that id exists again, TTL‑bounded (§6.1.8(B), §15.8).
> 2. **A rename chained onto an in‑flight rename.** The journal entry had a single retire slot, so the inherited intent was dropped with only a log line — leaving the *first* dag_id deployed, registered and unpaused beside the newest, under a banner reporting success. The entry now carries a **queue** and the reconciler drains it (idempotently, tracking what it already retired).
> 3. **A timed‑out rename's retire, orphaned by the next deploy.** `supersede` only scans open entries, so an `expired` entry's un‑performed retire was unreachable — while *Keep waiting* simultaneously refused it because a newer deploy was in flight, and the editor could no longer reach the expired deploy_id at all. A new deploy now adopts a recently‑terminal **budget‑skipped** retire of the same flow (never a *decided* skip) and marks it consumed.

> **Rename hardened — commit‑last, one action, one dag_id ✅ (2026‑08‑17).** A real rename destroyed a user's DAG: the reconciler registered the new DAG, **purged the old one**, then declared itself `superseded` — *"nothing further was done"* — with `unpause` and `trigger` still pending. The destructive step had run at position 2 of 4 and the completing steps never ran, leaving **zero** working DAGs.
>
> Three changes, and they ship together:
>
> 1. **Commit‑last (I1).** The single `retire` step is split. `quiesce` — *pause* the renamed‑away DAG, fully reversible — takes the old position, so the double‑scheduling hazard that motivated retiring early is still closed before the new DAG is unpaused. The irreversible half (delete the `.py`, optionally its history) moves to **last**, and `_ready_to_retire` refuses it unless `unpause` and `trigger` actually completed. A skip caused by a *timeout* never authorises destruction; only a deliberate skip (a pause veto, run‑on‑deploy off) does. An abandoned migration now leaves the old DAG paused and intact.
> 2. **No silent abandonment (I2).** Every terminal transition routes through one `_terminate`; if anything irreversible already committed, an abandon outcome (`superseded`/`expired`/`denied`/`cancelled`) is rewritten to **`needs_repair`**, names both dag_ids, and is always audited. The sentence *"nothing further was done"* is now reachable only when it is true.
> 3. **One action.** The three‑button *Keep history / Purge old DAG* modal is gone: renaming is type‑the‑name + Enter, always keep‑history. Purge was a co‑equal button one click from the routine case and is what destroyed the user's run history; it now lives where the user has evidence the new DAG works — the manager row's Delete. Retired ids are also filtered from `GET /dags` (`mark_retired`/`drop_retired`), self‑healing once Airflow marks the row stale.
>
> **Verified live, end to end:** `HELLOO → HELLO3` observed stepping `registered → quiesce → unpause → trigger → retire`, with the manager settling on a single `HELLO3`. Both invariants are pinned by regression tests built from the real incident.
