...
- **Schedule on assets (consumer side)** — a new DAG‑field `dag.schedule_assets: string[]` (edited as a comma‑separated **“schedule on assets”** text field in the DAG tab, like `tags`). When non‑empty it **overrides** the cron/preset `schedule` and codegen emits `schedule=[Asset('a'), Asset('b')]` (the DAG runs when **all** listed assets update). Empty → the existing time `schedule` is unchanged.
...
**Asset modelling:** each entry is a single string — an asset **name** or a **URI** — emitted as `Asset('<string>')` (verified against `apache‑airflow‑task‑sdk 1.2.2`: `Asset(name=None, uri=None, …)` requires at least one, and a lone positional sets both `name` and `uri`, the canonical Airflow‑3 form for either a plain name or a URI). `from airflow.sdk import Asset` is collected **only when** the DAG actually uses an asset (schedule, inlet, or outlet) — a DAG with no assets is byte‑unchanged (no stray import). Advanced asset surface — `AssetAlias`, `AssetAny`/`AssetAll` boolean schedules, watchers, `group`/`extra`, the `@asset` decorator — is deferred 🔭 (addable with no IR change). Wireframes **§15.1** (DAG tab schedule‑on‑assets field) + **§15.3** (NODE‑tab Assets section).
