...
- **Git `DeployTarget` ✅ (2026‑06‑23)** — commit/push generated DAGs to a git working tree an Airflow `GitDagBundle` tracks (`AIRFLOW_DEPLOY_TARGET=git`); **S3 / object‑storage** target later. Both DAG‑bundle aware, same `DeployTarget` interface (§6.5.1 / §8.7).
...
| **v1.2** | **Advanced KubernetesPodOperator surface ✅** (node_selector/labels/annotations/service_account_name/priority_class_name/security_context + pod_template_file/pod_template_dict declarative escape hatch for volumes/secrets/affinity/resources); **Asset / data‑aware scheduling ✅** (`dag.schedule_assets` → `schedule=[Asset(...)]` + per‑task `outlets`/`inlets`, §6.9); **Git `DeployTarget` ✅** (`GitDeployTarget`: commit/push to a git `GitDagBundle` repo, factory‑selected by `AIRFLOW_DEPLOY_TARGET`); S3 `DeployTarget` + per‑user identity + audit (remaining) |
