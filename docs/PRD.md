...
> **Known defect — the migration is browser‑resident (2026‑08‑15).** The whole post‑write lifecycle (*wait for registration → retire the old dag_id → unpause → trigger*) runs only inside the open `.afdag` widget: the gate is `StudioApp.tsx` `if (res.data.state === 'registered')`, the retire intent is a bare `React.useRef`, and the server stops at `target.write` with **no background work at all**. Close the tab, reload the page, or shut the browser while Airflow is still scanning and the migration is abandoned silently — the new DAG stays **paused** and never runs, and the old dag_id stays **deployed and active**.
>
> This is not the timeout path. Measured on a real occurrence: file written 18:56:09, Airflow registered it 19:01:05 (**296 s** — a full `dag_processor.refresh_interval`), poll deadline 19:03:09, page reloaded 18:59:42 — *85 s before registration*. **The earlier 420 s‑budget fix hardened a path that was never taken**, so it cannot prevent this class of failure. `find_orphans` is also blind to the result (it joins on `afdag_id`, and both files carry a live one), so nothing surfaces it.
>
> **Fix direction:** move completion server‑side — journal the pending `{dag_id, retire:{dag_id,purge}, run_on_deploy, deadline}` at deploy, reconcile it from a periodic callback, and let the banner *observe* rather than *perform*. Until then, a half‑finished rename is recoverable from the manager (retire keeps run history; purge does not).
