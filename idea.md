...
- [x] all operator should be able to handle variables **DONE 2026-08-12** — PRD §6.10 / §15.3a: a per-field `{ }` variable picker on every templatable operator field (one RJSF seam, no per-operator YAML change), inserting Jinja in fields and `Variable.get()` in code bodies.
